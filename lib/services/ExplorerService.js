class ExplorerService {
	...
	static getExplorersNamesByMission(explorers, mission) {
		const explorersInNodeToGetNames = explorers.filter(explorer => explorer.mission == mission);
		const namesInNode = explorersInNodeToGetNames.map(explorer => explorer.name);
		return namesInNode;
	}
}
